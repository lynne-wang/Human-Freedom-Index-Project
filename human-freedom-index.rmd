...
```{r set-warning-message-echo, warning=FALSE, message=FALSE, echo=FALSE}
...
We wish to isolate the relationships between freedom and other social and economic phenomena, as well as understand the ways in which different facets of freedom interact with each other. In particular, we will examine whether economic, and personal liberties can stand alone or must necessarily hinge on each other, and what their relationship is. We are also interested in the difference in overall and particular freedoms over time between Post-Soviet and Western/NATO countries, and how government size may affect freedom. Furthermore, we are interested in which attributes of freedom contribute the most to a country or region’s overall freedom, as well as which regions rank extremely in particular and overall freedom. Moreover, we would like to map the flux in global or regional freedom over the past few years. As such an appropriate research question is: What are the relationships between freedom and other social and economic factors and how do different facets of freedom interact with each other?
...
ggplot(freedom_map, mapping = aes(long, lat, group = group)) +
  ...
  labs(title = "Graph 2: Average Freedom Around the World", x = "Longitude", 
       y = "Latitude", fill = "Mean Human Freedom Index")
```
...
_Initial visualization of difference between economic and personal freedom_
...
_Bootstrap simulation based testing_
...
_Visualization of economic and personal freedom difference by region_
...
_Pearson correlation coefficient and visualization of relationship between economic and personal freedom_
...
ggplot(data = ef_govt_coldwar, 
       aes(x = as.factor(year), y = mean_ef_government, group = coldwar_side, 
           color = coldwar_side)) +
  geom_line(stat = "identity") +
...
ggplot(data = hf_coldwar, 
       aes(x = as.factor(year), y = mean_hf, group = coldwar_side, 
           color = coldwar_side)) +
  geom_line(stat = "identity") +
  labs(x = "Year", y = "Human Freedom Score", 
       title = 
         "Graph 7: Post-Soviet Countries Less Free Than Western Countries",
       subtitle = "2008 to 2017",
       color="Cold War Side")
```
...
_Linear regression of Cold War status and government size, in relation to overall freedom_
...
glance(m_hf_reg)$r.squared
```

_Linear regression of Cold War status and government size, in relation to personal freedom_
...
m_pf_reg <- lm(as.numeric(pf_score) ~ as.numeric(ef_government) + coldwar_side + coldwar_side * as.numeric(ef_government), 
               data = p_coldwar_govtsize_reg)

tidy(m_pf_reg) %>% 
  arrange(estimate) 

glance(m_pf_reg)$r.squared
```
...
ws_only <- ws_only%>%
  ...
  mutate(ws_score = (((pf_identity_divorce + pf_identity_sex_female)/2) + 
                       pf_ss_women + pf_movement_women)/3)
```

_Correlating women-specific freedom to overall freedom_
...
ggplot(data = ws_no_na, aes(x = ws_score, 
  ...
  labs(x = "Women-Specific Freedoms Score", y = "Overall Freedom Score", 
       title = "Graph 8: Positive Correlation Between Women-Specific and 
       Overall Freedom",
       subtitle = "Data from 162 countries from 2008-2017")
```
...
obs_diff_corr <- ws_no_na%>%
  summarize(obs_diff_corr = (cor(ws_score, pf_score_nowomen) - cor(ws_score, 
                                                                   ef_score)))
obs_diff_corr
```
...
_Bootstrap simulation difference in correlations_
...
$H_0$: There is no difference in the average correlation between w-s freedom to personal freedom and w-s freedom to economic freedom

$H_A$: There is a difference in the average correlation between w-s freedom to personal freedom and w-s freedom to economic freedom
...
for(i in 1:n_sims){ 
  ...
  boot_cor <- ws_no_na %>% 
    slice(indices) %>%                    
    summarize(boot_cor = (cor(ws_score, pf_score_nowomen) - cor(ws_score, 
                                                                ef_score))) %>% 
    pull()                                
...
```
There appears to be a general trend that regions with higher religious freedom score have higher women-specific freedom score, and vice versa. The Middle East and North Africa, which has the lowest religious freedom score, also has the lowest W-S freedom score. Sub-Saharan Africa does not fit with this pattern as clearly, with its moderate religious freedom score but a W-S score on the lower end. 


_Chi-squared test for independence_

In order to determine if the above average women-specific freedom score and average religious freedom score are associated, we performed a Chi-Squared test at the $\alpha$ = 0.05 significance level. Our null hypothesis is that there is no relationship between whether a country's women-specific freedom score is above or below average and whether their religious freedom score is above or below average. Our alternative hypothesis is that there is a relationship between the two, and the variables are not independent. We calculated the average freedom scores for each category and created a new variable for each that listed if the observation was above or below average for each criteria. 
...
Our p value is 0, which is less than our significance level of $\alpha$ = 0.01. This means we reject the null hypothesis, and found sufficient evidence to suggest that the difference in correlations between W-S freedom to personal freedom and W-S freedom to economic freedom was not zero. 

### Women-Specific Freedoms and Religious Freedom 
There appears to be a general trend that regions with higher religious freedom score have higher women-specific freedom score, and vice versa. The Middle East and North Africa region, which has the lowest religious freedom score, also has the lowest W-S freedom score. Sub-Saharan Africa does not fit with this pattern as clearly, with its moderate religious freedom score but a W-S score on the lower end. 
...
### DISCUSSION
_Economic and Personal Freedom Discussion_

Through our analysis, we observed that the relationship between economic and personal freedom was not as simple as we’d originally predicted. While we predicted that economic freedom was positively correlated with other freedoms, particularly personal freedom, the minute details showed us a more nuanced relationship. The Pearson correlation coefficient we calculated between economic freedom score and personal freedom score was 0.67, which indicated a moderate to strong positive correlation between the two variables; meanwhile, our bootstrap simulation test of the difference between economic and personal freedom score per country found a statistically significant difference in the average country’s economic and personal freedom score (p-value = 0 at the $\alpha$ = 0.05 significance level). The average economic freedom score was 0.32 points lower than the average personal freedom score. Moreover, our visualization of the average difference in economic and personal freedom score grouped by continent (graph 4) indicated that continents with a greater proportion of developed countries like Western Europe, Eastern Europe, and North America had significantly higher personal freedom scores compared to economic freedom scores, while continents with a greater proportion of developing countries like the Middle East & North Africa had significantly greater economic freedom scores compared to personal freedom scores. 

This raises an important question regarding whether unlimited freedom in all areas is necessarily optimal for the general well-being of the citizens of a country. For highly developed countries, it appears that having some restraints on economic freedom and a degree of government oversight, as opposed to a heavily laissez-faire model, can be beneficial to people’s personal freedoms. The Scandinavian countries, for instance, were able to uphold high personal freedom despite lower economic freedom. Meanwhile, the U.S. has higher economic freedom but relatively lower personal freedom (compared to many countries in Western Europe. This may be due to the U.S. government providing too much leeway to private companies to make higher profits, rather than guaranteeing individual citizens’ access to a stable base level of economic safety that would allow them greater personal freedoms. Perhaps in cases when economic and personal freedoms may come into friction with each other, the government should offer some amount of protection of personal freedoms instead of largely siding in favor with private enterprises. This may improve a country’s overall freedom, as well as the quality of life for a greater number of people in society. 

Regarding developing countries in the Middle East & North Africa, it appears that the relatively high economic freedom scores are related to the booming oil business in those countries. However, given the markedly lower personal freedom scores, it appears that the benefits of economic freedom are concentrated in the hands of only a few, likely the royal families and close private enterprises. We would dissuade government officials and researchers from viewing high economic freedom as a sign of a country’s overall freedom and well-being. We would like to emphasize that high economic freedom scores in and of themselves are not a cause for concern; rather, a developing country should take heed that personal freedom laws and regulations are not lagging too far behind in a period of rapid economic development.

_Cold War Status and Freedom Discussion_

Our analysis of the difference in freedom and government size between Post-Soviet and Western/NATO countries also provided some unexpected results. While our initial visualizations (graphs 6 and 7) aligned with our initial hypotheses that NATO countries would have higher overall freedom scores compared to Post-Soviet countries, we also found that Post-Soviet countries have smaller governments, on average, than NATO countries (higher ef_government score). Our linear regression analysis on the relationship between Cold War status, government size, and their interaction on overall human freedom score found that while NATO countries had higher baseline human freedom scores compared to Post-Soviet countries, smaller government has a sizeable positive relationship with overall human freedom score in Post-Soviet countries. Meanwhile, smaller government had a slight negative relationship with overall human freedom score in NATO countries. Moreover, our linear regression analysis on the relationship between Cold War status, government size, and their interaction on personal freedom score found that smaller government has a sizable positive relationship with personal freedom score in Post-Soviet countries. Meanwhile, smaller government had a sizable negative relationship with personal freedom score in NATO countries. Our findings are statistically significant because the p-values for the relevant variables we performed the regression on are less than 0 at the $\alpha$ = 0.05 significance level.

This suggests that perhaps there is no one-size-fits-all approach to how much government oversight is most beneficial for the citizens of a country. NATO countries have higher overall freedom scores compared to Post-Soviet countries, so it may be the case that some degree of government oversight on certain freedoms (i.e. economic ones designed mainly to benefit corporations) would allow for less interference to individuals’ personal freedoms. On the other hand, Post-Soviet countries have had a history of tighter state control, so relaxing that control would allow individuals less interference on their personal freedoms from strict government regulations. It appears that not enough regulation over private businesses is a greater threat to individuals’ personal freedom in NATO countries, while government overregulation over individuals’ lives is a more prominent threat to individuals’ personal freedom in Post-Soviet countries. Perhaps this could be extended to other countries as well: officials should carefully consider their governing approach depending on their country’s historical experiences with state control. In other words, limiting government presence might be beneficial from a baseline of high government control, while increasing government presence might be beneficial from a baseline of a more laissez-faire approach. 

_Women-Specific Freedom Discussion_

Great strides have been made globally and locally with regards to women's rights in the past century, even the past decade. However, around the world there are places where women's freedom is significantly hindered solely on the basis of their gender. Women may be refused access to political participation or education, die needlessly from childbirth, or just generally be prevented from making deeply personal choices in their private lives [1]. Though child brides and genital mutilation may be the extremes on this scale, exploring women's freedoms and their relationship to other kinds of freedoms remains relevant. 

It is not surprising to see that women-specific freedoms and overall human freedom are positively correlated. What is slightly more interesting, though, is the fact that economic freedom and personal freedom have different correlations to women-specific freedom. We investigated earlier that economic freedom scores tend to be lower than personal freedom scores around the world, but that doesn't translate into an expectation that the correlations would be different. As the researchers define it, individuals have economic freedom when "property they acquire without the use of force, fraud, or theft is protected from physical invasions by others and they are free to use, exchange, or give their property as long as their actions do not violate the identical rights of others.” At first glance, one might expect that this would strongly correlate with women's freedoms because women might have less property rights or be prohibited from certain economic activities. However, inheritance rights were included in the personal freedoms category. Thus, though all of these measures of freedom (personal, economic, women-specific) are generally positively correlated with each other, they don't have the same strength of correlation. 

We also saw in the above discussion that overall freedom is trending downward. If we know that women-specific freedoms and other freedoms are correlated, we might be able to investigate if some types of freedom, economic or personal, are more conducive to the spread and sustenance of other freedoms, a possible future direction that would require continuing to track freedom measurements over time. 

When considering countries that have fewer rights for women, it is also interesting to see how it is overall related to religious freedom, since there are examples where the two are (at least seemingly) linked. For example, in some Middle Eastern countries that practice Islam, there are laws that have burdened women in the family, the society, and the state[2]. They are prohibited from holding high political office, abortion, and divorce in some countries[2]. These are the some of the same countries that have low tolerance for other religions[3]. In the Human Freedom Index, this is correlated with lower scores for women's freedoms, and for religious freedom. It is important, however, to be sensitive to the fact that the values assigned in this study came from researchers in developed countries who may have a different perception on what negatively impacts women's freedoms than the women living in those situations. 

With this in mind, and the general trend thus far of freedoms being positively correlated with each other, it makes sense that women-specific freedoms and religious freedoms are not independent. When looking at why Sub-Saharan Africa does not follow this trend, and has a lower women-specific freedom score than expected, we identified that, although there are a wide variety of religions and practices people follow, women in Sub-Saharan Africa face significant sexual discrimination and abuse, intimate violence, political marginalization, and economic deprivation[4]. 

_Limitations_

In our analysis of the Human Freedom Index dataset, there are clear limitations that need to be recognized. One such limitation is that the data is not randomly selected and does not contain the randomness that is typically desired from a dataset. Another limitation is that though analysis was performed, it was done so in a manner that assumed the individual countries to be independent and absent of regional factors that affected each others score's in one or more categories. This presence of confounding variables somewhat decreases the validity of the analysis since it is easy to recognize that regional conflicts would affect certain freedom scores rather than solely the policies or type of government present in a given country. 

_Future Directions_

If this project were to continue in the future there are a few key areas that would require further attention. First, it is necessary to continue tracking changes in freedom over time. Likewise, it is critical that further possible relationships be identified and explored among the assortment of freedoms and other variables. Another key area relates to exploring the different relationshops between freedom levels at different levels of economic development. Lastly, it would be greatly beneficial to determine if certain types of freedom (economic or social) are more likely to enable or allow other freedoms to emerge or be sustained. 

### CITATIONS
1. https://blogs.worldbank.org/opendata/where-world-do-women-still-face-legal-barriers-own-and-administer-assets  
2. https://ips-dc.org/women_in_the_middle_east/  
3. https://www.wilsoncenter.org/article/us-report-religious-freedom-middle-east-and-north-africa  
4. https://www.du.edu/korbel/hrhw/researchdigest/africa/WomensRights.pdf
