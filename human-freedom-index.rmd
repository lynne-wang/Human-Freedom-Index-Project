...
ggplot(data = ws_no_na, aes(x = ws_score, 
  ...
  labs(x = "Women-Specific Freedoms Score", y = "Economic Freedom Score", 
       title = "Positive Correlation Between Women-Specific and Economic Freedom",
       subtitle = "Data from 162 countries from 2008-2017")
...
obs_diff_corr
```
hypoth test that the observed difference in correlations is nonzero 
...
boot_og <- boot_cors%>%
  summarize(lower = quantile(boot_distribution, 0.005), 
            upper = quantile(boot_distribution, 0.995))
boot_og
```
...
ggplot(data = boot_cors, aes(x = shifted_cors)) +
  geom_histogram(color = "darkblue", fill = "skyblue") + 
  labs(x = "Voice Jitter", 
       title = "Bootstrap Distribution of Difference in Correlations Centered at 0 appears Normal") +
